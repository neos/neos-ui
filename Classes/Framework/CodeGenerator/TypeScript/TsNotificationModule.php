...
#[Flow\Proxy(false)]
final class TsNotificationModule implements TsModuleInterface, TsRenderableInterface
{
    ...
    public function render(string $indent = '', bool $trimFirstLine = false): string
    {
        ...
        $lines[] = $indent . 'import { createNotification } from "@neos-project/framework-api";';
        $lines[] = '';
        ...
        $lines[] = $this->eventDto->renderTypeAlias('NOTIFICATION_TYPE', $indent, $trimFirstLine);
        $lines[] = '';

        $lines[] = $indent . 'export const ' . $this->exportName . ' = createNotification(';
        $lines[] = $indent . '    NOTIFICATION_TYPE,';
        $lines[] = $this->eventDto->renderSchema('NOTIFICATION_TYPE', $indent . '    ', false);
        $lines[] = $indent . ');';
        $lines[] = '';
