...
namespace Neos\Neos\Ui\ReferencesEditor\Application\GetTree;
...
final class GetTreeQueryHandler
{
        ...
        $treeBuilder = $nodeService->createTreeBuilderForRootNode(
            ...
            linkableNodeSpecification: new LinkableNodeSpecification(
                linkableNodeTypes: $nodeTypeService->createNodeTypeFilterFromNodeTypeNames(
                    nodeTypeNames: $query->allowedNodeTypes,
                ),
            ),
        );
    ...
    private function loadTree(
        ...
        if ($query->selectedNodeIds) {
            foreach ($query->selectedNodeIds as $selectedNodeId) {
                $selectedNodeAggregateId = NodeAggregateId::fromString($selectedNodeId);
                if ($treeBuilder->containsNodeTreeByNodeAggregateId($selectedNodeAggregateId) === false) {
                    $selectedNode = $nodeService->findNodeById($selectedNodeAggregateId);
                    if ($selectedNode) {
