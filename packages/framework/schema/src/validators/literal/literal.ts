...
export const literal = <L extends Primitive>(literalValue: L) =>
    createValidator<L>({
        description: `Literal(${describeValue(literalValue)})`,

        *test(valueUnderTest, { fail }) {
            if (valueUnderTest !== literalValue) {
                yield fail(
                    `${describeValue(
                        valueUnderTest
                    )} is not literally ${describeValue(literalValue)}.`
                );
            }
        },
    });
