...
test(`The reducer should return an Immutable.Map as the initial state.`, t => {
    ...
    t.true(nextState.get('ui').get('leftSideBar') instanceof Map);
});
...
test(`The reducer should initially mark the sidebar as visible.`, t => {
    ...
    t.false(nextState.get('ui').get('leftSideBar').get('isHidden'));
});
...
test(`
    ...
    t.true(nextState1.get('ui').get('leftSideBar').get('isHidden'));
    t.false(nextState2.get('ui').get('leftSideBar').get('isHidden'));
});
