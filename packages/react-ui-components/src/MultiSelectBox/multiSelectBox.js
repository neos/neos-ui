...
const MultiSelectBox = props => {
    const getOptionValueAccessor = () => {
        return $get([props.optionValueField]);
    };
    ...
    const optionValueAccessor = getOptionValueAccessor();

    const handleNewValueSelected = value => {
        const values = props.values || [];
        const updatedValues = [...values, value];
        props.onValuesChange(updatedValues);
    };
    ...
    return (
        <div className={classNames}>
            ...
            <SelectBox
                {...omit(props, ['theme', 'className'])}
                options={filteredSearchOptions}
                value=""
                onValueChange={handleNewValueSelected}
                disabled={disabled}
                />
        </div>
    );
